...
to execute-actions
  ...
  ;in order to do one action per tick. I don't know if that's actually what we want though
  ask turtle 0 [
    ifelse dirty?
      [ clean-dirt ]
      [ move ]
...
to move
  set-finished
  ifelse rotate?
    [ right 90 ] ;it says we can only turn 90 degrees right
    [ forward 1 ]
end
...
to set-finished
  if xcor = max-pxcor [
    ifelse (max-pycor mod 2) = 0
      [ set finished ycor = max-pycor ]
      [ set finished ycor = min-pycor ]
  ]
  if (xcor = min-pxcor) and (ycor = min-pycor) and (heading = 0)
    [ set finished false ]
end
...
to-report rotate?
  ifelse finished
    [ ifelse xcor = max-pxcor
        [ if heading != 90 [ report true ] ]
        [ if heading != 0 [ report true ] ]
    ]
    [ if ycor = min-pycor [
        ifelse ((xcor mod 2) = 0)
          [ if (heading != 0) [ report true ] ]
